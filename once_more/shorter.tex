...
We present here a novel algorithm
that is able to mine all closures
in order of descending support.

$\bullet$ \added{Troppus, taken from an "Algorithms" 
section that we had, compare to below and I guess simply
delete it here.}

\added{The text contained two versions, not identical
but essentially the same. --JLB}

\begin{algorithm}
\caption{{\sc Troppus}} % \label{algorithm}
  \begin{algorithmic}[1]
    \STATE $h = \{\overline{\emptyset}\}$ \COMMENT{add closure of $\emptyset$ to an empty heap}
    \WHILE {$h$ not empty} 
    	\STATE $X = h$.pop() \COMMENT{minimal element from the heap $h$}
